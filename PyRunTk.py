...
# Option to launch "Show keys" and/or "List Show" at startup
# Option to start command when clicking on show keys (skip enter)
...
Vars.ProgramVersionNumber.set('2.2.0')
...
def ReturnCallback(event):
    ...
    if '****' in command: return; #ignore spacer lines
...
#Entry4Var = StringVar()
Entry4 = Entry(Main)#, textvariable=Entry4Var)
Entry4.pack(side=TOP, fill=X, expand=TRUE)
...
# ------------------------------
def NotReadyYet():
    command = tkinter.messagebox.showerror("Not ready yet","Selected option not ready yet. Sorry!")
...
OptionsMenu.add_checkbutton(label='Enter not needed',  onvalue=1, offvalue=0, variable=Vars.EnterNotNeeded)
Vars.EnterNotNeeded.set(1)
OptionsMenu.add_command(label='Verify URLs', command=NotReadyYet)
OptionsMenu.add_checkbutton(label='List Show at startup', onvalue=1, offvalue=0, variable=Vars.ListShowStartup)
Vars.ListShowStartup.set(1)
OptionsMenu.add_checkbutton(label='Keys Show at startup', onvalue=1, offvalue=0, variable=Vars.KeysShowStartup)
